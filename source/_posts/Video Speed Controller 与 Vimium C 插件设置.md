---
title: Video Speed Controller 与 Vimium C 插件设置

time: 20-39

date: 2022-04-12 20:39:59
...
调整下 Video Speed Controller视频速度控制器 和 Chrome 浏览器 插件 Vimium C 的快捷键设置

想办法wasd不仅控制视频，而且还要控制网页。

无语哔哩哔哩网页版快捷键太坑了，改天把它禁掉。wasd现阶段废了，

视频还是用ijkl控制(YouTuBe自带的视频快捷键挺好用的)

<!--more-->

# Video Speed Controller视频速度控制器

发现YouTube自带的快捷键挺好用的。

![img](https://wangguanjingji.oss-cn-beijing.aliyuncs.com/picture/Snipaste_2022-04-12_21-12-32.png)

# Vimium C 设置

这个的好好重新设置一下了，

{% spoiler "Vimium C设置me" %}
...
map cc copyCurrentUrl #将当前URL复制到剪贴板
map nn moveTabToNewWindow #将选项卡移到新窗口
map dd togglePinTab #固定或取消固定当前标签
map qq Vomnibar.activateInNewTab
map ff openCopiedUrlInNewTab #在新标签页中打开剪贴板

map ? showHelp #显示帮助
map g LinkHints.activate #在新标签页中打开链接并切换到该链接
map Q Vomnibar.activateInNewTab # 搜索在新标签中打开URL，书签或历史记录(可以充当默认搜索)


map fq Vomnibar.activateTabSelection #搜索已打开的标签页
map fw visitPreviousTab #转到上次访问的标签
map fe focusInput #输入gNi前焦点集中到第N个输人框
map fa goToRoot #跳转到父页面的根目录
map fs Vomnibar.activateEditUrlInNewTab #编辑当前URL并在新选项卡中打开
map fd togglePinTab

map s scrollDown    #向下滚动
map w scrollUp      #向上滚动
map W scrollToTop   #滚动到顶部
map S scrollToBottom   #滚动到底部
#map <a-s-s> scrollPageDown    #向下滚动半个页面的高度
#map <a-s-w> scrollPageUp      #向上滚动半个页面的高度

map x removeTab #关闭当前标签页
map X restoreTab #恢复当前标签页

#Vim快捷键补充工具
shortcut userCustomized1 command="removeTab" #-关闭标签页

#map fs Vomnibar.activateBookmarksInNewTab #在新标签页中打开书签
#map <c-d> duplicateTab #复制当前标签
#map cl closeTabsOnLeft #关闭左侧的标签页
#map cr closeTabsOnRight #关闭右侧的标签页
#map ca closeOtherTabs #关闭所有其他标签页
#map mm Marks.activateCreateMode #创建一个新标记 相当于锚点
#map ` Marks.activateGotoMode #转到标记
#map [[ goPrevious #跟进标有上一个或‘<’的链接【分页网站】暂时不知道啥意思
#map ]] goNext #跟进标有下一个或‘>’的链接【分页网站】
```

{% endspoiler %}

{% spoiler "Vimium C设置系统" %}
...
map z restoreTab
map x removeTab goto="previous" highlighted
map c previousTab
map v nextTab
...
bl: http://www.bilibili.com/search?keyword=$s
```

{% endspoiler %}

{% spoiler "Vimium C搜索引擎系统" %}
